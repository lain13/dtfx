...
{
    /// <summary>
    /// MSSQLデータ取得処理(SqlSelect)
    /// MSSQLサーバーからデータを取得してその結果をファイル又は新たな一時テーブルに出力する。
    /// </summary>
    public class SqlSelectExecutor : ExecutorBase
    {
        public override ResultTypeCode Execute(XElement rawElement)
        {
            ...
            using (var command = new SqlCommand(element.Value))
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        throw new AppConfigurationException(XSqlElementConstants.ElementName.SqlSelect, "XML形式が正しくありません。" + rawElement.ToString());
                    }
                }
            }
        ...
        private void WriteToTable(SqlDataReader reader, SqlSelectElement element)
        {
            MethodBase method = MethodInfo.GetCurrentMethod();
        ...
        /// <param name="reader">SqlDataReader</param>
        /// <param name="element">SqlSelectElement</param>
        private void WriteToCSV(SqlDataReader reader, SqlSelectElement element)
        {
            MethodBase method = MethodInfo.GetCurrentMethod();
        ...
        /// <param name="reader">SqlDataReader</param>
        /// <param name="element">SqlSelectElement</param>
        private void WriteToVariable(SqlDataReader reader, SqlSelectElement element)
        {
            MethodBase method = MethodInfo.GetCurrentMethod();
