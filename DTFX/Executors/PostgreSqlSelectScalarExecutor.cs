/************************************************************************
* ファイル名:	PostgreSqlSelectScalarExecutor.cs
* 概要: 
* 履歴:
*	バージョン		日付		作成者		内容
...
{
    /// <summary>
    /// PostgreSQLデータ取得処理(PostgreSQLSelectScalarExecutor)
    /// PostgreSQLサーバーからデータを取得してその結果を変数に出力する。
    /// </summary>
    public class PostgreSqlSelectScalarExecutor : ExecutorBase
    {
        public override ResultTypeCode Execute(XElement rawElement)
        {
            ...
            using (var command = new NpgsqlCommand(element.Value))
            {
                ...
                {
                    throw new AppConfigurationException(XSqlElementConstants.ElementName.PostgreSqlSelectScalar, "XML形式が正しくありません。" + rawElement.ToString());
                }
            }
