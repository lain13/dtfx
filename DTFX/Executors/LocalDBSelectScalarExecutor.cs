/************************************************************************
* ファイル名:	LocalDBSelectScalarExecutor.cs
* 概要: 
* 履歴:
*	バージョン		日付		作成者		内容
...
{
    /// <summary>
    /// 一時DBデータ取得処理(LocalDBSelectScalarExecutor)
    /// 一時DBサーバーからデータを取得してその結果を変数に出力する。
    /// </summary>
    public class LocalDBSelectScalarExecutor : ExecutorBase
    {
        public override ResultTypeCode Execute(XElement rawElement)
        {
            ...
            using (var command = new SqlCommand(element.Value))
            {
                ...
                {
                    throw new AppConfigurationException(XSqlElementConstants.ElementName.LocalDBSelectScalar, "XML形式が正しくありません。" + rawElement.ToString());
                }
            }
        ...
        /// <returns>LocalDBSelectScalarElement</returns>
        public LocalDBSelectScalarElement CreateElement(XElement rawElement)
        {
            LocalDBSelectScalarElement obj = new LocalDBSelectScalarElement();
        ...
        /// <param name="element">LocalDBSelectScalarElement</param>
        private void WriteToVariable(object obj, LocalDBSelectScalarElement element)
        {
            MethodBase method = MethodInfo.GetCurrentMethod();
