/************************************************************************
* ファイル名:	PostgreSqlBulkInsertFromOracleExecutor.cs
* 概要: 
* 履歴:
*	バージョン		日付		作成者		内容
...
{
    /// <summary>
    /// PostgreSqlデータ一括登録処理(PostgreSqlBulkInsertFromOracle)
    /// Oracleサーバーからデータを取得してその結果をPostgreSqlサーバーに出力する。
    /// </summary>
    public class PostgreSqlBulkInsertFromOracleExecutor : ExecutorBase
    {
        public override ResultTypeCode Execute(XElement rawElement)
        {
            ...
            {
                throw new AppConfigurationException(XSqlElementConstants.ElementName.PostgreSqlBulkInsertFromOracle, "データソースが同の場合は利用できません。" + rawElement.ToString());
            }
            ...
            using (var command = new OracleCommand(element.Value))
            {
                ...
                using (OracleDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        throw new AppConfigurationException(XSqlElementConstants.ElementName.PostgreSqlBulkInsertFromOracle, "XML形式が正しくありません。" + rawElement.ToString());
                    }
                }
            }
        ...
        /// <summary>
        /// XElementからPostgreSqlBulkInsertFromOracleElementを生成します。
        /// </summary>
        /// <param name="rawElement">XElement</param>
        /// <returns>PostgreSqlBulkInsertFromOracleElement</returns>
        public PostgreSqlBulkInsertFromOracleElement CreateElement(XElement rawElement)
        {
            PostgreSqlBulkInsertFromOracleElement obj = new PostgreSqlBulkInsertFromOracleElement();
        ...
        /// <param name="element">PostgreSqlBulkInsertFromOracleElement</param>
        private void WriteToTable(IDataReader reader, PostgreSqlBulkInsertFromOracleElement element)
        {
            MethodBase method = MethodInfo.GetCurrentMethod();
