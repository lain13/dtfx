...
ソリューションの `x86` 構成では、実行ファイルを `x86`、アーキテクチャに依存しない `IF.Batch.Common` を `Any CPU` としてビルドします。Common は `x86` の Debug / Release でもビルド対象に含まれます。構成ごとの対応は [`docs/common-library.md`](docs/common-library.md#ビルド構成) を参照してください。
...
CSV 読み込み、共有変数の反復、ZIP 作成まで試す場合は次を実行します。

```bat
examples\csv-pipeline\CSV_PIPELINE.BAT
```

各サンプルの入力、生成物、カスタマイズ方法は [`examples/README.md`](examples/README.md) を参照してください。
...
スモークテストは、結果コードの優先順位、コマンドライン引数の解析、CSV 形式、式評価、Serilog ファイル出力、XSD のコンパイル、および全サンプルのスキーマ整合性を検証します。実データベースへの接続テストは含みません。
...
examples/csv-pipeline/   CSV 読み込みと ZIP 作成の実行例
...
- [IF.Batch.Common リファレンス](docs/common-library.md)
- [実行可能なサンプル](examples/README.md)
