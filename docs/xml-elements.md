...
`If` は `test` の式を JScript として評価し、結果が文字列 `true` の場合に子要素を実行します。任意の `toVariable` を指定すると評価結果を保存します。

```xml
<If test="${user_count} &gt; 0" toVariable="has_users">
  <TraceLog eventType="information">Users found.</TraceLog>
</If>
```

XML 属性内で論理 AND を書く場合は `&amp;&amp;`、比較の `<` は `&lt;` と記述してください。
